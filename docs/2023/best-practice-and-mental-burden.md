...
其实在一直以来的 hui 开发与维护过程中，我发现我给很多开发者带来很多心智负担。比如，

+ 这个 element-ui 有但 hui 没有。
+ hui-pro 这个这个不支持，怎么办？
+ 这个 hui 没有，我做不了。

举个实际的例子，当我认为业务可能会产生一些通用的需求的时候，那我会封装一些通用的东西，比如 `@hui-pro/utils` 有个功能是专门做正则表达式的；当时的需求确实是因为大家都需要做校验。而这个功能刚好适用于当下的所有前端项目。
...
当然不仅仅是我，其他人也会产生心智负担给我，比如：我最早来公司的时候，结一个缺陷单，必选项只有一个。而现在必选项茫茫多。
...
另一个建议，不要过度封装，尽可能原子化；所以我做了 code-fun ，vue2-sfc，当然社区也有很多好的实践，比如 [codepen](https://codepen.io/)， [vue-playground](https://sfc.vuejs.org/) 等。如果一定要做一些 `npm` 或者工具的封装，尽可能保持解耦，也可以考虑下 `tailwindcss` 的实现，他的好处就是当你使用它的时候，其实你就在学习真正的 css 了，而不是仅仅在学习它的 API。
